...
void printingShortestRoute(int *parents, int start, int end, int totalV, unsigned int sd) 
{
...
unsigned int Dijkstra(Vertex *vertices, int start, int end, int totalV, int totalE)
{
	...
	printingShortestRoute(parents, start, end, totalV, sd); 
...
void readQueries(Vertex *vertices, int totalV, int totalE, char *filename) 
{
	...
	for (i = 0; i < totalQ; i++) {
		...
		Dijkstra(vertices, start, end, totalV, totalE);
