...
unsigned int relax(Vertex *vertices, int u, int v, int unsigned distanceU, unsigned int *distances, int *parents) 
{
...
void initializeSingleSource(Vertex *vertices, int totalV, int start, unsigned int *distances, int *parents, Candidate *C, int *totalCandidates)
{
	...
	for (i = 0; i < vertices[start].adjacentCount; i++) {
		...
		C[*totalCandidates].distance = relax(vertices, start, v, 0, distances, parents);
	...
	while (length != totalV) {
		...
		if (u == end) {
			...
			for (i = 0; i < vertices[u].adjacentCount; i++) {
				...
				C[totalCandidates].distance = relax(vertices, u, v, distances[u], distances, parents);
				C[totalCandidates++].index = v;
			}
