...
// Functions related to building the adjacency matrix
...
// Including functions from heap implementation after candidate declaration
//#include "heapImplementation.c"
// Heap Functions
...
unsigned int Dijkstra(Candidate *C, unsigned int *distances, int *parents, int *route, Vertex *vertices, int start, int end, int totalV, int totalE)
{
	...
	//checkHeapProperty(C, totalCandidates);

	//printC(C, totalCandidates);

	heapify(C, totalCandidates);

	//checkHeapProperty(C, totalCandidates);

	//fprintf(stdout, "printing before\n");
	//printC(C, totalCandidates);
	...
	//while (length != totalV) {
		...
		//fprintf(stdout, "printing before extractions\n");
		//printC(C, totalCandidates);

		int u = extractMinHeap(C, &totalCandidates);
		//checkHeapProperty(C, totalCandidates);
		//fprintf(stdout, "chosen = %d totalCandidates = %d\n", u, totalCandidates);

		if (u == end) {
			totalCandidates = 0;
			//fprintf(stdout, "this happened\n");
		}
		else {
			//printC(C, totalCandidates);
			...
			for (i = 0; i < vertices[u].adjacentCount; i++) {
				...
				if (change != 0) {
					insertHeap(C, &totalCandidates, v, distances[v]);
					//fprintf(stdout, "total Candidates = %d\n", totalCandidates);
				}
			}
		}
...
void initializeSingleSource(Vertex *vertices, int totalV, int start, unsigned int *distances, int *parents, Candidate *C, int *totalCandidates)
{
	...
	//fprintf(stdout, "what is going on? %d\n", *totalCandidates);
	//printC(C, *totalCandidates);
}
...
void readQueries(Vertex *vertices, int totalV, int totalE, char *filename) 
{
	...
	Candidate *C = (Candidate *)malloc(sizeof(Candidate) * totalE);
...
void insertHeap(Candidate *C, int *totalCandidates, int index, int indexDistance)
{
	C[*totalCandidates].distance = indexDistance;
